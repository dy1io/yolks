...
## === ENVIRONMENT VARS ===
# STARTUP, STARTUP_PARAMS, STEAM_USER, STEAM_PASS, SERVER_BINARY, MOD_FILE, MODIFICATIONS, SERVERMODS, OPTIONALMODS, UPDATE_SERVER, VALIDATE_SERVER, VALIDATE_MODS , MODS_LOWERCASE, STEAMCMD_EXTRA_FLAGS, STEAMCMD_APPID, SERVER_PASSWORD, STEAMCMD_ATTEMPTS, DISABLE_MOD_UPDATES
...
if [[ -f ${MODLIST} ]] && [[ -n "$(cat ${MODLIST} | grep 'Launcher')" ]]; then
        ...
        do
            if [[ $modID =~ ^[0-9]+$ ]]; then # Only check mods that are in ID-form
                ...
                if [[ ! -d $modDir ]] || [[ ${VALIDATE_MODS} == 1 ]] || [[ ! -f $modDir/timestamp ]] || [[ ( -n $latestUpdate ) && ( $latestUpdate =~ ^[0-9]+$ ) && ( $latestUpdate > $(find $modDir/timestamp | head -1 | xargs stat -c%Y) ) ]]; then
                    ...
                    echo -e "\tWaiting 10 seconds for rate limit cooldown...\n"
                    sleep 10
...
if [[ ! -f ./${SERVER_BINARY} ]]; then
    echo -e "\n${RED}[STARTUP_ERR]: Specified server binary could not be found in the root directory!${NC}"
...
${modifiedStartup} 2>&1 | tee ${ServerLogsLocation}
